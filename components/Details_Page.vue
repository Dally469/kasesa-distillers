<template>
      ...
      <div class="py-7 bg-[#262022] text-white w-full md:w-[30%]">
        ...
        <div class="m-10">
          ...
          <div class="flex flex-wrap gap-4 justify-center">
            <NuxtLink class="font-bold cursor-pointer hover:text-red-500">#Crafts</NuxtLink>
            <NuxtLink class="font-bold cursor-pointer hover:text-red-500">#Flights</NuxtLink>
            <NuxtLink class="font-bold cursor-pointer hover:text-red-500">#Hop</NuxtLink>
            <NuxtLink class="font-bold cursor-pointer hover:text-red-500">#Wheat</NuxtLink>
          </div>
        </div>
      </div>
  ...
  />
        ...
        <div class="grid gap-5 p-4 grid-cols-1 sm:grid-cols-2">
          <div v-for="item in paginatedProducts" :key="item.id" class="w-full">
            <div class="flex flex-col items-center p-4 bg-white rounded-lg shadow-lg">
              ...
              <div class="pt-5">
                <button class="bg-red-500 py-3 px-9 font-semibold rounded-full hover:bg-gray-300 duration-300">Add Cart</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="Products.length > productsPerPage" class="flex justify-between mt-5">
          ...
          <UButton  @click="nextPage" :disabled="currentPage * productsPerPage >= Products.length" icon="i-heroicons-arrow-long-right"/>
        </div>
      </div>
    </div>
  </div>
